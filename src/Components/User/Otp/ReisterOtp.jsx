...
import { sendEmailAndOtp, submitUserDetailsOtp } from '../../../config/clientEndPoints'
...
function ReisterOtp() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (joinedOtp.length === 4) {
     const response = await sendEmailAndOtp(userData,joinedOtp)
  ...
  const handleResend=async()=>{
    try {
      const response = await submitUserDetailsOtp(userEmail)
      if(response.data.success){
        setCount(5)
        toast.success(response.data.message)
      }else{
